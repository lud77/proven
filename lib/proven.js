...
const hasDocumentationFields = (homepage, man) => homepage || man;

const defaultMessages = ({ maxAge, minMaintainers, minVersions, minDownloads, period }) => ({
    ...
    hasDocumentation: `Module ${chalk.yellowBright(`doesn't have a documentation link`)}`,
    minDownloads: `Module has ${chalk.yellowBright(`less than ${minDownloads} downloads in the ${period.replace('-', ' ')}`)}`
});

const applyRule = ({ maxAge, minMaintainers, minVersions, repoRequired, allowedLicenses, docsRequired, minDownloads }) => (data) => ({
    ...
    hasDocumentation: !docsRequired || hasDocumentationFields(data.homepage, data.man),
    minDownloads: minDownloads == -1 || data.downloads > minDownloads
});

const processNpmData = ([stats, downloads]) => ({
    age: moment().diff(moment(stats.time.modified), 'days'),
    maintainers: stats.maintainers.length,
    versions: R.toPairs(stats.versions).length,
    license: stats.license,
    repository: stats.repository !== undefined,
    downloads: downloads.downloads
});

const processModules = (moduleStats) => (limits) => Promise.map(moduleStats, processModule(limits));

const processModule = (limits) => ([name, version, stats, downloads]) =>
    Promise.map([stats, downloads], parseJson)
        .then(processNpmData)
