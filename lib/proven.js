...
const removeIgnored = (ignoreList) => (pair) => ignoreList.includes(pair[0]);

const processIgnoreList = (ignoreFile) => ignoreFile
    .then((ignoreString) => ignoreString.toString('utf8').split('\n'))
    .then(R.map(R.trim));

const extractLimits = (configJson, defaultLimits) => {
    if (!configJson) return defaultLimits;
